...
export default async function BlogPostPage({ params }: BlogPostPageProps) {
  ...
  // Get related posts (excluding current post) — only for sidebar layout
  const relatedPosts = useFullLayout
    ? []
    : allPosts
        .filter(p => p.slug !== slug)
        .slice(0, 3)

  // Get all unique tags from all posts — only for sidebar layout
  const allTags = useFullLayout ? [] : Array.from(new Set(allPosts.flatMap(p => p.tags)))

  const contentArea = (
    <div className={useFullLayout ? 'w-full px-4 lg:w-8/12' : 'w-full px-4 lg:w-8/12'}>
      <div>
  ...
  return (
    <section className="pb-[120px] pt-[150px]">
      <div className="container">
        <div className={`-mx-4 flex flex-wrap ${useFullLayout ? 'justify-center' : ''}`}>
          {contentArea}
          {!useFullLayout && (
